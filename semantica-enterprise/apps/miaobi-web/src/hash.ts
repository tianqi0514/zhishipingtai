...
/**
 * SHA-256 without a secure-context dependency.
 *
 * Web Crypto deliberately hides `crypto.subtle` on plain HTTP origins.  The
 * platform is frequently deployed on an isolated-network IP before TLS is
...
export async function sha256(value: unknown): Promise<string> {
  ...
  for (let offset = 0; offset < message.length; offset += 64) {
    for (let index = 0; index < 16; index += 1) {
      schedule[index] = messageView.getUint32(offset + index * 4);
    }
