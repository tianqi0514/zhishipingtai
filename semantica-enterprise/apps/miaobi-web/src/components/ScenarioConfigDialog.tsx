...
import { api } from '../api';
import { ChapterKnowledgeSettings, type KnowledgeSetting } from './ChapterEvidence';
...
export function ScenarioConfigDialog({ packageId, versionId, onClose, onSaved, onError }: {
  packageId: string;
  versionId?: string;
  onClose: () => void;
  ...
  useEffect(() => {
    api<ConfigResponse>(`/writing/scenario-packages/${packageId}/business-config${versionId ? `?version_id=${versionId}` : ''}`)
      .then(setData)
