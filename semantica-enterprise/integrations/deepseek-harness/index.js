...
export function apply(ctx) {
      ...
      return {
        ...
        messages: [
          ...
          createUserMessage({
            content: [{
              type: 'text',
              text: '本章所需的项目上下文、章节资料包和真实文档证据已经齐备。现在停止检索，严格按照本轮正式报告 JSON 契约输出唯一根对象 {"sections":[{...}],"warnings":[]}。sections 必须是数组且本轮只有一个章节对象；绝不能把 section_key、title、content_nodes、依赖数组或 status 平铺到根层。不得输出工作过程、工具名或额外说明。',
            }],
  ...
  registerTool(defineTool({
    ...
    description: '读取一个已激活业务对象的安全语义属性、指标固定口径、关系路径、映射状态和数据新鲜度。响应只包含业务语义 ID，不包含物理表字段；指标的 default_aggregate、required_filters 与 required_relationships 属于受信口径。',
    parameters: {
  ...
  registerTool(defineTool({
    ...
    description: '提交严格的语义查询计划和 Query IR。必须采用 structured_get_object 返回的 default_aggregate、business_definition、required_filters 与 required_relationships，并把固定筛选同时写入 Plan filters 和 IR where。平台会在受信边界强制注入固定筛选和去重安全的关联 EXISTS 口径，再验证权限和映射、确定性编译只读 SQL、参数绑定执行并返回可核验数据引用；不得传入 SQL 或物理名称。成功一次后直接使用结果，不要对等价映射重复查询。',
    parameters: {
