...
jq -nc '{canonical_name:"传神智库3D验收节点",aliases:["验收节点","传神智库验收节点"],confidence:0.99}' \
  | api -X PUT -H 'Content-Type: application/json' -d @- "${api_base}/knowledge/entities/${node_a}" >/dev/null
...
jq -nc '{predicate:"联合建设",confidence:0.96}' \
  | api -X PUT -H 'Content-Type: application/json' -d @- "${api_base}/knowledge/facts/${edge_id}" >/dev/null
api -X DELETE "${api_base}/knowledge/facts/${edge_id}" >/dev/null
...
cascade_edge="$(jq -nc --arg space "$space_id" --arg source "$node_b" --arg target "$node_a" '{space_id:$space,subject_entity_id:$source,predicate:"运营",object_entity_id:$target,confidence:1}' \
  | api -H 'Content-Type: application/json' -d @- "${api_base}/knowledge/facts" | jq -r '.id')"
removed="$(api -X DELETE "${api_base}/knowledge/entities/${node_b}" | jq -r '.removed_facts')"
